...
func ProxyingMiddleware(ctx context.Context, instances string, logger log.Logger) services.ServiceMiddleware {
...
func (mw proxymw) Uppercase(s string) (string, error) {
	response, err := mw.uppercase(mw.ctx, requests.UppercaseRequest{S: s})
	if err != nil {
		return "", err
	}

	resp := response.(responses.UppercaseResponse)
	if resp.Err != "" {
...
func makeUppercaseProxy(ctx context.Context, instance string) endpoint.Endpoint {
	...
	return httptransport.NewClient(
		"GET",
		u,
		transportUtils.EncodeRequest,
		responses.DecodeUppercaseResponse,
	).Endpoint()
}
